...
class TransitionTimeProcessor(UseCaseProcessor):
    ...
    current_activity_col_attr_descriptor = AttributeDescriptor(
        ...
        display_name="target activity table column",
        description="Activity table column value at time of target activity",
    )

    previous_activity_col_attr_descriptor = AttributeDescriptor(
        attribute_type=dynamic_attributes.CurrentActivityColumnAttribute,
        display_name="source activity table column",
        description="Activity table column value at time of source activity",
    )
    ...
    def _gen_dynamic_attr_list(self, min_attr_count: int, max_attr_count: int):
        ...
        if (
            ...
            previous_activity_col_attributes = (
                feature_processor_new.gen_previous_activity_col_attributes(
                    process_config=self.process_config,
                    activity_table_str=self.activity_table_str,
                    suffix="(at source event)",
                    columns=self.considered_activity_table_cols,
                    is_feature=True,
                    is_class_feature=False,
                )
            ...
            dynamic_attributes_list = (
                dynamic_attributes_list + previous_activity_col_attributes
            )
...
class StartActivityProcessor(UseCaseProcessor):
    ...
    potential_dynamic_attributes_descriptors = [current_activity_col_attr_descriptor]
    # the target attribute that is used for this use case
    target_attribute_descriptor = AttributeDescriptor(
        attribute_type=static_attributes.SpecificStartActivityAttribute,
        display_name=static_attributes.SpecificStartActivityAttribute.display_name,
        description=static_attributes.SpecificStartActivityAttribute.description,
    )
    ...
    def process(self):
        ...
        target_attribute = static_attributes.SpecificStartActivityAttribute(
            process_config=self.process_config,
            activity_table_str=self.activity_table_str,
        ...
        activity_col_str = self.process_config.table_dict[
            self.activity_table_str
        ].activity_col_str
        filter_first_activity = feature_processor_new.filter_first_activity(
            activity_table_str=self.activity_table_str, activiy_col_str=activity_col_str
        )
        self.filters.append(filter_first_activity)
        # Get DataFrames
        self.df_x, self.df_target = feature_processor_new.extract_dfs(
            process_config=self.process_config,
            activity_table_str=self.activity_table_str,
...
class TransitionProcessor(UseCaseProcessor):
    """Feature Processor for the transition violation (when is an activity followed
    by a target activity and when not?) use case."""
    ...
    # the target attribute that is used for this use case
    target_attribute_descriptor = AttributeDescriptor(
        attribute_type=dynamic_attributes.SpecificNextActivityAttribute,
        display_name=dynamic_attributes.SpecificNextActivityAttribute.display_name,
        description=dynamic_attributes.SpecificNextActivityAttribute.description,
    )
    ...
    def process(self):
        ...
        target_attribute = dynamic_attributes.SpecificNextActivityAttribute(
            process_config=self.process_config,
            activity_table_str=self.activity_table_str,
            next_activity=self.target_activity,
            is_feature=False,
            is_class_feature=True,
        )
...
class IncompleteCaseProcessor(UseCaseProcessor):
    """Feature Processor for the activity use case."""
    ...
    # the target attribute that is used for this use case
    target_attribute_descriptor = AttributeDescriptor(
        attribute_type=static_attributes.IncompleteCaseAttribute,
        display_name=static_attributes.IncompleteCaseAttribute.display_name,
        description=static_attributes.IncompleteCaseAttribute.description,
    )
    ...
    def process(self):
        ...
        target_attribute = static_attributes.IncompleteCaseAttribute(
            process_config=self.process_config,
            activity_table_str=self.activity_table_str,

...
def date_filter_PQL(process_config: ProcessConfig, activity_table_str: str,
    ...
    if start_date:
        ...
        filter_str = (f'PU_FIRST("{case_table_str}", '
...
def all_cases_closed_query(process_config: ProcessConfig, activity_table_str: str,
                           name="IS_CLOSED"):
    """Return PQLColumn that evaluates to 1 for all cases with default column name
    IS_CLOSED"""
    activity_table = process_config.table_dict[activity_table_str]
    q_str = (f'CASE WHEN PU_COUNT("{activity_table.case_table_str}", '
             f'"{activity_table.table_str}"."'
             f'{activity_table.activity_col_str}") >= 1 THEN 1 ELSE 0 END')
    return PQLColumn(name=name, query=q_str)


def compute_min_max_attribute_counts_PQL(min_counts_perc: float, max_counts_perc: float,
                                         process_config: ProcessConfig,
                                         activity_table_name: str, chunksize: int,
                                         filters: List[PQLFilter]) -> Tuple[int, int]:
    ...
    """
    activity_table = process_config.table_dict[activity_table_name]
    query_num_cases = (
            'COUNT(DISTINCT "' + activity_table_name + '"."' +
            activity_table.caseid_col_str + '")')
    pql_num_cases = PQL()
    pql_num_cases.add(PQLColumn(query_num_cases, "number cases"))
    df_num_cases = get_df_with_filters(process_config.dm, filters, pql_num_cases,
                                       chunksize=chunksize)
...
def gen_static_activity_occurence_attributes(process_config: ProcessConfig,
                                             activity_table_str: str,
                                             activities: List[str] = None,
                                             min_vals: int = 0, max_vals: int = np.inf,
                                             is_feature: bool = True,
                                             is_class_feature: bool = False, ) -> List[
    ActivityOccurenceAttribute]:
    ...
    if activities is None:
        ...
        activities = get_valid_vals(table_name=activity_table_str,
                                    column_names=[activity_table.activity_col_str],
                                    process_config=process_config, min_vals=min_vals,
                                    max_vals=max_vals, )[
            activity_table.activity_col_str]

    activity_occ_attributes = []
    for activity in activities:
        attr = ActivityOccurenceAttribute(process_config=process_config,
...
def extract_dfs(process_config: ProcessConfig, activity_table_str: str,
                static_attributes: List[StaticAttribute],
                dynamic_attributes: List[DynamicAttribute],
                is_closed_indicator: PQLColumn, target_variable: PQLColumn,
                filters: List[PQLFilter], ) -> Tuple[Any, ...]:
    ...
    if dynamic_attributes:
        ...
        extractor = PQLExtractor(process_model=process_model,
        ...
        df_x, df_y = extractor.get_closed_cases(process_config.dm,
                                                only_last_state=False)
    else:
        extractor = StaticPQLExtractor(process_model=process_model,
                                       target_variable=target_variable,
                                       is_closed_indicator=is_closed_indicator,
                                       static_features=static_attributes_pql, )
        df_x, df_y = extractor.get_closed_cases(process_config.dm,
...
def get_valid_vals(table_name: str, column_names: List[str],
                   process_config: ProcessConfig, min_vals: int = 0,
                   max_vals: int = np.inf, filters: List[PQLFilter] = None,
                   chunksize: int = 10000):
    ...
    for col_name in column_names:
        query.add(PQLColumn(name=col_name + "_values",
                            query='DISTINCT("' + table_name + '"."' + col_name +
                                  '")', ))
        query.add(PQLColumn(name=col_name + "_count",
                            query='COUNT_TABLE("' + target_table_str + '")',
                            ))  # Can just query  # enough occurences using filter

    df_val_counts = get_df_with_filters(dm=process_config.dm, filters=filters,
                                        query=query, chunksize=chunksize)
    ...
    for col in cols:
        ...
        valid_vals_dict[col_name] = df_val_counts[
            (df_val_counts[count_col_name] >= min_vals) & (
                    df_val_counts[count_col_name] <= max_vals)][col].values.tolist()
...
class FeatureProcessor:
    ...
    def __init__(self, process_config: ProcessConfig, chunksize: int = 10000,
        ...
        (self.min_attr_count,
         self.max_attr_count,) = self.compute_min_max_attribute_counts_PQL(
    ...
    def get_valid_vals(self, table_name: str, column_names: List[str],
                       process_config: ProcessConfig, min_vals: int = 0,
                       max_vals: int = np.inf, ):
        """Get values of columns that occur in enough cases. This is done with PQL
        and shall be used before the actual column is queried for the resulting
    ...
    def _gen_dynamic_activity_occurence_attributes(self, process_config: ProcessConfig,
                                                   activity_table_str: str,
                                                   activities: List[str] = None,
                                                   min_vals: int = 0,
                                                   max_vals: int = np.inf,
        ...
        activity_table = process_config.table_dict[activity_table_str]
        if activities is None:
            activities = self.get_valid_vals(table_name=activity_table.table_str,
                                             column_names=[
                                                 activity_table.activity_col_str],
        ...
        activity_occ_attributes = []
        for activity in activities:
            attr = PreviousActivityOccurrenceAttribute(process_config=process_config,
                                                       activity_table_str=activity_table_str,
                                                       activity=activity,
    ...
    def _gen_static_activity_occurence_attributes(self, process_config: ProcessConfig,
                                                  activity_table_str: str,
                                                  activities: List[str] = None,
                                                  min_vals: int = 0,
                                                  max_vals: int = np.inf,
        ...
        if activities is None:
            ...
            activities = self.get_valid_vals(table_name=activity_table_str,
                                             column_names=[
                                                 activity_table.activity_col_str],
                                             process_config=process_config,
                                             min_vals=min_vals, max_vals=max_vals, )[
    ...
    def _gen_static_numeric_activity_table_attributes(self,
                                                      process_config: ProcessConfig,
                                                      activity_table_str: str,
                                                      columns: List[str],
                                                      aggregation: str,
        ...
        for column in columns:
            attr = NumericActivityTableColumnAttribute(process_config=process_config,
                                                       activity_table_str=activity_table_str,
                                                       column_name=column,
    ...
    def _gen_numeric_case_column_attributes_single_table(self,
                                                         process_config: ProcessConfig,
                                                         table_str: str,
                                                         columns: List[str],
                                                         is_feature: bool = True,
    ...
    def _gen_categorical_case_column_attributes_single_table(self,
                                                             process_config:
                                                             ProcessConfig,
                                                             table_str: str,
                                                             columns: List[str],
    ...
    def _gen_case_column_attributes_multi_table(self, process_config: ProcessConfig,
        ...
        for table_name, all_columns in table_column_dict.items():
            ...
            cat_cols, num_cols = self.process_config.get_categorical_numerical_columns(
                table_name)
            ...
            num_attributes = self._gen_numeric_case_column_attributes_single_table(
                process_config=process_config, columns=num_cols, is_feature=is_feature,
    ...
    def _default_target_query(self, process_config: ProcessConfig,
                              activity_table_str: str, name="TARGET"):
    ...
    def extract_dfs(self, process_config: ProcessConfig, activity_table_str: str,
                    static_attributes: List[StaticAttribute],
                    dynamic_attributes: List[DynamicAttribute],
                    is_closed_indicator: PQLColumn, target_variable: PQLColumn, ) -> \
            Tuple[Any, ...]:
    ...
    def run_total_time_PQL(self, activity_table_str: str,
                           considered_activity_table_cols: List[str],
                           considered_case_level_table_cols: Dict[str, List[str]],
                           time_unit="DAYS", start_date: Optional[str] = None,
        ...
        date_filter_PQL(self.process_config, activity_table_str, start_date, end_date)
        # Define closed case (currently define that every case is a closed case)
        is_closed_indicator = all_cases_closed_query()
        ...
        static_attributes = [WorkInProgressAttribute(process_config=self.process_config,
                                                     activity_table_str=activity_table_str,
                                                     aggregation="AVG", is_feature=True,
                                                     is_class_feature=False, ),
                             StartActivityAttribute(process_config=self.process_config,
                                                    activity_table_str=activity_table_str,
                                                    is_feature=True,
                                                    is_class_feature=False, ),
                             EndActivityAttribute(process_config=self.process_config,
                                                  activity_table_str=activity_table_str,
                                                  is_feature=True,
                                                  is_class_feature=False, ),
                             StartActivityTimeAttribute(
        ...
        activity_occ_attributes = self._gen_static_activity_occurence_attributes(
            ...
            is_feature=True, min_vals=self.min_attr_count,
            max_vals=self.max_attr_count, )
        (_,
         num_actiivty_table_cols,) = \
            self.process_config.get_categorical_numerical_columns(
            activity_table_str)
        ...
        num_actiivty_table_cols = utils.list_intersection(num_actiivty_table_cols,
                                                          considered_activity_table_cols)

        numeric_activity_column_attributes = (
            self._gen_static_numeric_activity_table_attributes(
                process_config=self.process_config,
                activity_table_str=activity_table_str, columns=num_actiivty_table_cols,
        ...
        static_attributes = (
                static_attributes + activity_occ_attributes +
                numeric_activity_column_attributes + case_table_column_attrs)
        ...
        target_attribute = CaseDurationAttribute(process_config=self.process_config,
                                                 activity_table_str=activity_table_str,
        ...
        # Get DataFrames
        self.df_x, self.df_target = self.extract_dfs(process_config=self.process_config,
                                                     activity_table_str=activity_table_str,
        ...
        pp = PostProcessor(df_x=self.df_x, df_target=self.df_target,
                           attributes=static_attributes + dynamic_attributes,
                           target_attribute=target_attribute, valid_target_values=None,
                           invalid_target_replacement=None, min_counts_perc=0.02,
                           max_counts_perc=0.98, )

        self.df_x, self.df_target, self.target_features, self.features = pp.process()

        statistics_computer = StatisticsComputer(features=self.features,
    ...
    def run_decision_point_new(self, source_activity: str, target_activities: List[str],
                               time_unit="DAYS", start_date: Optional[str] = None,
                               end_date: Optional[str] = None,
        ...
        """
        # Add filters for start and end date
        date_filter_PQL(start_date, end_date)

        static_attributes = [
            StartActivityAttribute(process_model=self.process_model, is_feature=True,
                                   is_class_feature=False, ),
            StartActivityTimeAttribute(process_model=self.process_model),
            EndActivityTimeAttribute(process_model=self.process_model),
            CaseDurationAttribute(process_model=self.process_model,
                                  time_aggregation=time_unit, is_feature=False,
                                  is_class_feature=False, ),
        ...
        categorical_case_table_column_attrs = (
            self._gen_categorical_case_column_attributes_single_table(
                columns=self.static_categorical_cols, is_feature=True,
                is_class_feature=False, ))
        numeric_case_table_column_attrs = self._gen_numeric_case_column_attributes(
        ...
        static_attributes = (
                static_attributes + categorical_case_table_column_attrs +
        ...
        dynamic_attributes = (
                dynamic_attributes + num_act_cols + cat_act_cols +
                activity_occ_attributes)
        ...
        target_attribute = NextActivityAttribute(process_model=self.process_model,
                                                 is_class_feature=True,
                                                 attribute_name="Transition to "
                                                                "activity", )
        ...
        self.df_x, self.df_target = self.extract_dfs(
            static_attributes=static_attributes, dynamic_attributes=dynamic_attributes,
        ...
        pp = PostProcessor(df_x=self.df_x, df_target=self.df_target,
                           ...
                           min_counts_perc=0.02, max_counts_perc=0.98, )
    ...
    def _filter_next_activity(self, next_activity: str):
        pql_str = (f'ACTIVITY_LEAD("{self.process_model.activity_table_str}"."'
    ...
    def _filter_prev_activity(self, prev_activity: str):
        pql_str = (f'ACTIVITY_LAG("{self.process_model.activity_table_str}"."'
                   f"{self.process_model.activity_column_str}\", 1) = '"
                   f"{prev_activity}'")
    ...
    def run_transition_time(self, source_activity: str, target_activity: str,
                            time_unit="DAYS", start_date: Optional[str] = None,
                            end_date: Optional[str] = None, ):
        """Run feature processing for total case time analysis.
        ...
        dynamic_attributes = (
                dynamic_attributes + current_num_act_cols + current_cat_act_cols +
                prev_num_act_cols + prev_cat_act_cols)
        ...
        target_attribute = ActivityDurationAttribute(process_model=self.process_model,
                                                     unit=time_unit, is_feature=False,
                                                     is_class_feature=True, )
    ...
    def set_latest_date_PQL(self):
        query = PQL()
        query += PQLColumn(
            "MAX(" + '"' + self.activity_table_name + '"' + "." + '"' +
            self.eventtime_col + '"' + ")",
            "latest_date", )
        df = self.dm.get_data_frame(query, chunksize=self.chunksize)
        self.latest_date = df["latest_date"][0]

    def _conv_dtypes_PQL(self, df: pd.DataFrame, src_dtypes: List[str],
                         target_dtype: str) -> pd.DataFrame:
    ...
    def compute_min_max_attribute_counts_PQL(self, min_counts_perc: float,
                                             max_counts_perc: float) -> Tuple[int, int]:
        """Compute the minimum and maximum required attribute counts.
        ...
        """
        query_num_cases = (
                'COUNT(DISTINCT "' + self.activity_table_name + '"."' +

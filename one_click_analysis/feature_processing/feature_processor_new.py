...
def all_cases_closed_query(
    process_config: ProcessConfig, activity_table_str: str, name="IS_CLOSED"
):
...
def compute_min_max_attribute_counts_PQL(
    min_counts_perc: float,
    max_counts_perc: float,
    process_config: ProcessConfig,
    ...
    df_num_cases = get_df_with_filters(
        process_config.dm, filters, pql_num_cases, chunksize=chunksize
    )
...
def gen_static_activity_occurence_attributes(
    process_config: ProcessConfig,
    activity_table_str: str,
...
def extract_dfs(
    ...
    if dynamic_attributes:
        ...
        df_x, df_y = extractor.get_closed_cases(
            process_config.dm, only_last_state=False
        )
    else:
...
def gen_static_numeric_activity_table_attributes(
    process_config: ProcessConfig,
    activity_table_str: str,
    columns: List[str],
    aggregation: str,
    ...
    :param columns: columns to use for activity table. Can also contain categorical
    columns. These are then not considered.
    :param aggregation:
    :param is_feature:
    ...
    selected_num_actiivty_table_cols = utils.list_intersection(
        num_actiivty_table_cols, columns
    )
    attrs = []
    for column in selected_num_actiivty_table_cols:
...
class FeatureProcessor:
    ...
    def _gen_case_column_attributes_single_table(
        self,
        process_config: ProcessConfig,
        table_str: str,
        columns: List[str],
    ...
    def _gen_case_column_attributes_multi_table(
        ...
        for table_name, all_columns in table_column_dict.items():
            ...
            (
                cat_cols,
                num_cols,
            ) = self.process_config.get_categorical_numerical_column_names(table_name)
            # Now only use those columns that are defined in the table_column_dict
            cat_cols = utils.list_intersection(cat_cols, all_columns)
            num_cols = utils.list_intersection(num_cols, all_columns)
            # Get attributes for categorical columns
    ...
    def run_total_time_PQL(
        ...
        (
            _,
            num_actiivty_table_cols,
        ) = self.process_config.get_categorical_numerical_column_names(
            activity_table_str
        )
        ...
        num_actiivty_table_cols = utils.list_intersection(
            num_actiivty_table_cols, considered_activity_table_cols
        )
        ...
        case_table_column_attrs = self._gen_case_column_attributes_multi_table(
            process_config=self.process_config,
            table_column_dict=considered_case_level_table_cols,
            is_feature=True,
            is_class_feature=False,
        )
        """

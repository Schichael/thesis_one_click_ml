...
class AnalysisTransitionViolation:
    ...
    def _create_description(self):
        static_attributes = TransitionProcessor.potential_static_attributes_descriptors
        dynamic_attributes = (
            TransitionProcessor.potential_dynamic_attributes_descriptors
        )
        name_str = "Activity violation Analysis"
        goal_str = (
            "The goal of the transition violation analysis is to get insights "
            "into the possible root causes of directly-follows transitions that are "
            "not defined "
            "in the process model. Each occurrence of the source activity is used "
            "for analysis. It is analyzed whether the next activity is the target "
            "activity or not. <br>One case can have multiple occurrences of the source "
            "activity. So, it is possible that multiple transitions from a single "
            "case are used for analysis. <br>Only closed cases are used for this "
            "analysis."
        )
        definition_str = (
            "A violation of a transition occurs that is not allowed according to the "
            "process model."
        )
    ...
    def run_analysis(self, out: widgets.Output):
        ...
        df_combined = self.transition_processor.df_x
        df_combined[
            self.transition_processor.df_target.columns.tolist()
        ] = self.transition_processor.df_target
